...
export default function Aurora({
  colorStops = ["#157d78", "#d88e36", "#c9688a"],
  amplitude = 1.0,
  blend = 0.5,
  time,
  speed = 1.0,
  ...
  useEffect(() => {
    ...
    const update = (tick: number) => {
      ...
      const nextTime = propsRef.current.time ?? tick * 0.01;
      if (!program) {
        return;
      }

      program.uniforms.uTime.value = nextTime * (propsRef.current.speed ?? 1.0) * 0.1;
      program.uniforms.uAmplitude.value = propsRef.current.amplitude ?? 1.0;
      program.uniforms.uBlend.value = propsRef.current.blend ?? blend;
      program.uniforms.uColorStops.value = (propsRef.current.colorStops ?? colorStops).map(
        (hex: string) => {
          const color = new Color(hex);
          return [color.r, color.g, color.b];
        },
